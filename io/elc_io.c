...
int read_and_check_elc_header(el_file_ptr file, const magic_number magic, version_number *version, const char* filename)
{
	...
	{
		LOG_ERROR_OLD("File '%s' too small! %d, %d", filename, size, sizeof(elc_file_header));
		return -1;
	}
	...
	if (memcmp(header.magic, magic, sizeof(magic_number)) != 0)
	{
		...
		LOG_ERROR_OLD("Wrong file '%s'! Magic number not correct! Should be \"%s\", but is \"%s\"!", filename, m_str, hm_str);
