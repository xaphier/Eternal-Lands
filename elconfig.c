...
int engine_shadow_quality = 0;
int engine_shadow_map_size = 2;
...
void change_engine_shadow_map_size(int* var, int value)
{
	*var = value;
	engine_set_shadow_map_size(*var);
}

void change_engine_shadow_distance(float* var, float* value)
{
	*var = *value;
	engine_set_shadow_distance(*var);
}

void change_engine_view_distance(float* var, float* value)
{
	*var = *value;
...
void change_engine_set_terrain_quality(int* var, int value)
{
	*var = value;
	engine_set_terrain_quality(*var);
}
		...
		if (config_dir != NULL)
		{
			...
			if (name_buf != NULL)
			{
				...
				if (unlink(name_buf) != 0)
					LOG_ERROR_OLD("%s: Failed to remove [%s] [%s]\n", __FILE__, rotate_chat_log_flag_file, strerror(errno) );
				else
					LOG_TO_CONSOLE(c_green2, rotate_chat_log_restart_str);
				free(name_buf);
			}
