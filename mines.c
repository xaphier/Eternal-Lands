...
char * get_mine_e3d(mine_type)
{
	...
	LOG_ERROR_OLD("Invalid mine type was requested!\n");
	return "";
}
...
void put_mine_on_ground(int mine_x, int mine_y, int mine_type, int mine_id)
{
	...
	if (!get_tile_valid(mine_x, mine_y))
	{
		...
		LOG_ERROR_OLD("A mine was placed OUTSIDE the map!\n");
		return;
	}
...
void add_mines_from_list (const Uint8 *data)
{
	...
	for (i = 0; i < mines_no; i++)
	{
		...
		{
			// Warn about this error!
			LOG_ERROR_OLD("A mine was located OUTSIDE the map!\n");
			continue;
		}
...
void remove_mine(int which_mine)
{
	...
	if (mine_list[which_mine].obj_3d_id == -1) {
		...
		LOG_ERROR_OLD("Oops, double-removal of mine!\n");
		return;
	}
	...
	for (def = node->children; def; def = def->next)
	{
		if (def->type == XML_ELEMENT_NODE)
		{
			if (xmlStrcasecmp (def->name, (xmlChar*)"mine") == 0)
			{
				...
				{
					LOG_ERROR_OLD("%s: Maximum mine types reached: %d", mines_config_error, MAX_MINE_DEFS);
					ok = 0;
				}
			...
			{
				LOG_ERROR_OLD("%s: Unknown element found: %s", mines_config_error, def->name);
				ok = 0;
			}
		}
...
void load_mines_config()
{
	...
	{
		LOG_ERROR_OLD("%s: No XML root element found in '%s'", mines_config_error, file);
	}
	// Is the root the right type?
	else if (xmlStrcmp(root->name, (xmlChar*)"mines"))
	{
		LOG_ERROR_OLD("%s: Invalid root element '%s' in '%s'", mines_config_error, root->name, file);
	}
