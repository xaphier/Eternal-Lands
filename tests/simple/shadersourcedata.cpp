...
BOOST_AUTO_TEST_CASE(shader_source_data_default_creation)
{
	...
	BOOST_CHECK_EQUAL(shader_source_data.get_material_texture_arrays(),
		false);
}
...
BOOST_AUTO_TEST_CASE(shader_source_data_source)
{
	...
	shader_source_data.set_source(el::String(L"3sfd23"));
	BOOST_CHECK_EQUAL(el::string_to_utf8(
		shader_source_data.get_source()), "3sfd23");

	shader_source_data.set_source(el::String(L"w4tgi3"));
	BOOST_CHECK_EQUAL(el::string_to_utf8(
		shader_source_data.get_source()), "w4tgi3");
}
...
BOOST_AUTO_TEST_CASE(shader_source_data_options)
{
	...
	shader_source_data.set_material_texture_arrays(false);
	BOOST_CHECK_EQUAL(shader_source_data.get_material_texture_arrays(),
		false);

	shader_source_data.set_material_texture_arrays(true);
	BOOST_CHECK_EQUAL(shader_source_data.get_material_texture_arrays(),
		true);
}
...
BOOST_AUTO_TEST_CASE(shader_source_parameter_save_load_xml)
{
	...
	shader_source_data_save.set_source(el::String(L"3sfd23"));
	...
	shader_source_data_save.set_material_texture_arrays(true);
	...
	BOOST_CHECK_EQUAL(el::string_to_utf8(
		shader_source_data_load.get_source()), "3sfd23");
	...
	BOOST_CHECK_EQUAL(shader_source_data_load.get_material_default(),
		false);
	BOOST_CHECK_EQUAL(shader_source_data_load.get_material_texture_arrays(),
		true);
}
