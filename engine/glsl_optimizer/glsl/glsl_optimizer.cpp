...
static void
initialize_mesa_context(struct gl_context *ctx, gl_api api, int opengl_version)
{
   ...
   ctx->Const.MaxTextureUnits = 16;
   ctx->Const.GLSLVersion = opengl_version;
   ...
   ctx->Const.VertexProgram.MaxAttribs = 32;
   ctx->Const.VertexProgram.MaxUniformComponents = 1024;
   ctx->Const.MaxVarying = 10;
...
struct glslopt_ctx {
	glslopt_ctx (bool openglES, bool opengl3) {
		mem_ctx = ralloc_context (NULL);
		initialize_mesa_context(&mesa_ctx, openglES ? API_OPENGLES2 : API_OPENGL, opengl3 ? 130 : 120);
	}
	~glslopt_ctx() {
		ralloc_free (mem_ctx);
	}
...
glslopt_ctx* glslopt_initialize (bool openglES, bool opengl3)
{
	return new glslopt_ctx(openglES, opengl3);
}
