...
namespace eternal_lands
{
	...
	class CodecManager
	{
		private:
			class GlFastLoadInfo
			{
				...
				public:
					...
					inline void set_red_mask(const Uint32 red_mask)
					{
						m_red_mask = red_mask;
					}

					inline void set_blue_mask(const Uint32 blue_mask)
					{
						m_blue_mask = blue_mask;
					}

					inline void set_green_mask(const Uint32 green_mask)
					{
						m_green_mask = green_mask;
					}

					inline void set_alpha_mask(const Uint32 alpha_mask)
					{
					...
					inline void set_swap_size(const Uint32 swap_size)
					{
			...
			class GlPackedPixelType
			{
				...
				public:
					GlPackedPixelType(const Uint32 red_mask,
						...
						const Uint32 alpha_mask, const GLenum type);
					bool has_alpha() const;
					GlFastLoadInfo build(const GLenum format,
						const bool swap_colors, const bool ignore_alpha)
						const;
			};

			std::vector<GlFastLoadInfo> m_flast_load_infos;

			void add_packed_gl_pixel_type(const GlPackedPixelType &pixel_type);
			void add_red_gl_pixel_type(const GLenum type, const Uint32 mask);
			void add_red_green_gl_pixel_type(const GLenum type, const Uint32 mask);
			void add_color_gl_pixel_type(const GLenum type, const Uint32 mask);
			void add_color_alpha_gl_pixel_type(const GLenum type, const Uint32 mask);
		...
		public:
			...
			/**
			 * Tries to find a combination of opengl format and type that matchs the
			 * given color bit masks. If a maching combination one is found, true is
			 * returned. If no matching combination is found, false is returned.
			...
			/**
			 * Tries to find a combination of opengl format and type that matchs the
			 * given color bit masks. If a maching combination is found, true is
			 * returned and the parameter gl_type, gl_format, size and swap_size are
			 * set with the correct values. If no matching combination is found, only
			 * false is returned.
			 * @param red_mask The bit mask for red.
			 * @param green_mask The bit mask for green.
			 * @param blue_mask The bit mask for blue.
			 * @param alpha_mask The bit mask for alpha.
			 * @param type If fast load is supported, this parameter is filled with
			 * the needed opengl type.
			 * @param format If fast load is supported, this parameter is filled with
			 * the needed opengl format.
			...
			bool is_fast_load_supported(const Uint32 red_mask,
				const Uint32 green_mask,
				const Uint32 blue_mask,
				const Uint32 alpha_mask, GLenum &type,
				GLenum &format, Uint32 &size,
				Uint32 &swap_size) const;

			/**
			 * Tries to find for a combination of opengl format and type the given
			 * color bit masks. If a maching combination is found, true is
			 * returned and the parameter red_mask, green_mask,  blue_mask, alpha_mask,
			 * size and swap_size are set with the correct values. If no matching
