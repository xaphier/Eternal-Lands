	...
	namespace dds
	{
			...
			const Uint32 blue_mask, const Uint32 alpha_mask, DdsHeader &header)
		{
			...
			if (depth > 0)
			{
				...
				header.m_caps.m_caps1 = DDSCAPS_COMPLEX | DDSCAPS_TEXTURE;
			...
			if (alpha_mask > 0)
			{
				header.m_pixel_format.m_flags |= DDPF_ALPHAPIXELS;
			}

			header.m_pixel_format.m_fourcc = 0;

			bpp = __builtin_popcount(red_mask | green_mask | blue_mask | alpha_mask);
