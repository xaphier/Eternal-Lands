...
namespace eternal_lands
{
	...
	namespace
	{
		...
		const ShaderTextureTypeData shader_texture_datas[] =
		{
		...
		const Uint32 shader_texture_datas_count =
			sizeof(shader_texture_datas) /
	...
	{
		if (shader_texture_datas_count <= texture)
		{
			EL_THROW_EXCEPTION(InvalidParameterException()
				<< errinfo_range_min(0)
				<< errinfo_range_max(
					shader_texture_datas_count - 1)
		...
		return shader_texture_datas[texture].get_name();
	}
		...
		const String &str)
	{
		...
		for (i = 0; i < shader_texture_datas_count; ++i)
		{
			shader_texture_type =
		...
		const ShaderTextureType texture)
	{
		...
		return shader_texture_datas[texture].get_layer_index();
	}
